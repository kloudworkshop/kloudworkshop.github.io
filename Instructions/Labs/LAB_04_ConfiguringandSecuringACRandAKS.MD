...
On the node pool tab click the link **agentpool**,

In Availbility Zones deselect all zones, in the scale method set
...
Select 'Update' to update node pool
...
7. Click **Next: Integrations >** and, on the **Integrations** tab accept all the default and press next, On the **Monitoring** clear to check boxes for **Enable Prmetheus metrics** and **Enable recommended alert rules**. 
